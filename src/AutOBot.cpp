...
void AutOBot::begin(DriveType IdriveType, int Ideviation) {
    this->driveType = IdriveType;
    this->deviation = Ideviation;
    if(driveType == DRIVE_DIFFERENTIAL || driveType == DRIVE_MECANUM) {
...
void AutOBot::drive(float x, float y, float w) {
    int vx = constrain(x, -100, 100);
    int vy = constrain(y, -100, 100);
    int vw = constrain(w, -100, 100);
